    ...
    <div className="p-4 space-y-4">
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="rounded-lg bg-secondary/50 p-3 space-y-1 text-sm">
            <div className="flex justify-between"><span className="text-muted-foreground">Materials</span><span>{formatPula(materialsCost)}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Labor ({workHours.toFixed(1)}h × {formatPula(job.costing.hourlyRate)})</span><span>{formatPula(laborCost)}</span></div>
            <div className="flex justify-between font-medium pt-1 border-t border-border"><span>Subtotal</span><span>{formatPula(subtotal)}</span></div>
          </div>

          <div>
            <Label className="text-xs">Hourly Rate (P)</Label>
      ...
      <Card className="border-primary/30 bg-primary/5">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-4xl font-bold text-primary">{formatPula(quotePrice)}</p>
          <p className="text-xs text-muted-foreground mt-1">Includes {job.costing.profitMargin}% profit ({formatPula(profitAmount)})</p>
        </CardContent>
      </Card>
